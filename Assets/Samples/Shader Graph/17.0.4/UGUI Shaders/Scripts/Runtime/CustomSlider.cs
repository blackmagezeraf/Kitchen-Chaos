...
public class CustomSlider : Selectable, IDragHandler, IInitializePotentialDragHandler, IMaterialModifier {
  ...
  private static Vector2 FromDirection(Direction direction) => direction switch {
  ...
  private static readonly string StatePropertyName       = "_State";
  private static readonly string SliderValuePropertyName = "_SliderValue";

  private          Material _material;
  private readonly float    _stepSize    = 0.1f;
  private          Vector2  _rangeOffset = Vector2.zero;

  private Direction direction => _direction;
  ...
  private int StatePropertyId {
    get {
      if (!_statePropertyId.HasValue)
        _statePropertyId = Shader.PropertyToID(StatePropertyName);
  ...
  private int SliderValuePropertyId {
    get {
      if (!_sliderValuePropertyId.HasValue)
        _sliderValuePropertyId = Shader.PropertyToID(SliderValuePropertyName);
  ...
  private Vector2 DirectionVector => FromDirection(direction);

  private Vector3 Vector => new Vector3(Value, DirectionVector.x, DirectionVector.y);
  ...
  private void UpdateDrag(PointerEventData eventData, Camera cam) {
    if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(rectTransform, eventData.position, cam,
  ...
  public override void OnMove(AxisEventData eventData) {
    ...
    case MoveDirection.Left:
      if (axis == Axis.Horizontal && FindSelectableOnLeft() == null)
        Value = reverseValue ? Value + _stepSize : Value - _stepSize;
      else
        base.OnMove(eventData);
      break;
  ...
  private static void CreateToggleGameObject(MenuCommand command) {
    GameObject go = ObjectFactory.CreateGameObject("Slider", new Type[] {
      typeof(RectTransform), typeof(CanvasRenderer), typeof(Image),
      typeof(RectTransformSize), typeof(CustomSlider)
    });
